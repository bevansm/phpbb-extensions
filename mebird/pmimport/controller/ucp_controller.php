...
namespace mebird\pmimport\controller;

/**
 * PM Import UCP controller.
 */
class ucp_controller
{
	...
	public function display_options()
	{
		...
		add_form_key('mebird_pmimport_ucp');
		...
		$data = array(
			'user_pmimport' => $this->request->variable('user_pmimport', $this->user->data['user_pmimport']),
		);
		...
		if ($this->request->is_set_post('submit'))
		{
			...
			if (!check_form_key('mebird_pmimport_ucp'))
			{
			...
			if (empty($errors))
			{
				...
				$message = $this->language->lang('UCP_PMIMPORT_SAVED') . '<br /><br />' . $this->language->lang('RETURN_UCP', '<a href="' . $this->u_action . '">', '</a>');
		...
		$this->template->assign_vars(array(
			...
			'S_USER_PMIMPORT'	=> $data['user_pmimport'],
		));
	}
